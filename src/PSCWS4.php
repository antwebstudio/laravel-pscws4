...
class PSCWS4
{
    ...
    public function __construct(Repository $config) {
        ...
        //设置编码和编码规则
        $charset = $config->get('pscws4.charset');
        $this->set_charset($charset);
        $this->set_dict(storage_path('app/etc/dict.xdb'));
        if (strtolower($charset) === 'gbk') {
            $rules = storage_path('app/etc/rules.ini');
        } elseif(strtolower($charset) === 'utf8' || strtolower($charset) === 'utf-8') {
            $rules = $rules = storage_path('app/etc/rules.utf8.ini');
        } else {
            throw new \Exception('Please Set Your Charset To "gbk" Or "utf8"');
        }
        $this->set_rule($rules);
    }
